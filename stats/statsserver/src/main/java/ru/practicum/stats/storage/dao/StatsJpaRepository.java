...
public interface StatsJpaRepository extends JpaRepository<Stats, Long> {
    @Query(value =
            "select new ru.practicum.dto.StatsOutDto(s.application, s.requestUrl, count( distinct s.addressIp) as c) " +
    ...
    @Query(value =
            "select new ru.practicum.dto.StatsOutDto(s.application, s.requestUrl, count( s.addressIp) as c) " +
                    "from Stats s where s.requestTime between ?1 and ?2 " +
                    "group by s.application, s.requestUrl " +
                    "order by c desc")
